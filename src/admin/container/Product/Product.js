...
function Product(props){
  ...
  const columns = [
    ...
    {
      field: 'prodact_img',
      headerName: 'prodact_img',
      renderCell: (params) => {
        console.log(params);

        if (params.row.prodact_img && params.row.prodact_img.url) {
          return <img src={params.row.prodact_img.url} alt={params.row.name} width={50} />;
        } else {
          return null;
        }
      },
        ...
        <form onSubmit={handleSubmit} method="post" enctype="multipart/form-data">
          <DialogContent>
            ...
            <input
             ...
             name="prodact_img" 
             onBlur={handleBlur}
            onChange={(event)=>setFieldValue("prodact_img",event.target.files[0])}
            />
            {
              values.prodact_img&&
              <img src={values.prodact_img.url?values.prodact_img.url:URL.createObjectURL(values.prodact_img)} alt={values.prodact_img.name} width={50} />
